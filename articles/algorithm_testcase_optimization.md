...
title: "テストケースは実装しているアルゴリズムの特性も加味して決める
			~素数判定アルゴリズムを例に、最適なテストケースについて考える~"
emoji: "🐺"
...
## 1. アルゴリズムの実装の違いで、実行時間の差を確認するつもりが、実行が一瞬で終わってしまった
...
**何と、最も時間がかかると思われたプログラムが、一瞬で計算を終えてしまった**
...
# Miller-Rabin (上のと同じ関数を使う場合は省略OK)
def is_prime_mr(n):
    if n < 2:
        return False
    small_primes = [2,3,5,7,11,13,17,19,23]
...
そもそも、`Miller-Rabin法`や`Pollard's Rho法`が何なのか、という話もありますが、話が難しくなるので、本記事では紹介しません。
