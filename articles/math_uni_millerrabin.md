...
となり、$7$ で割り切れる差をもつため、**$4$ と $-3$ は $mod\>7$では同じ数として扱える**ことを意味します。
...
## 2. Miller-Rabin素数判定法のアルゴリズム
入力を、素数を判定する整数 $n$, 出力を $n$ が素数の可能性があるとき $True$, 素数でないとき $False$ とします。

このとき、次のステップにより、$n$ が素数であるか否かを判定します。

1. $n > 2$ かつ、$n$ が偶数ならば、$False$ を出力し、終了する
2. $n - 1 = 2^st$ となる整数 $s >= 0$ と奇数 $t >= 3$ を見つける
3. $a \in {1, 2, 3,..., num - 1}$ をランダムに選ぶ
4. $a^t \equiv 1\> mod\> n$ の場合、$True$ を出力し、終了する
5. $i = 0$ とし、$i$ を $1$ ずつ増やしながら、$i < s$ の間以下を繰り返す
    (i) $a^{2^st} \equiv -1\> (mod\> n)$ の場合、$True$ を出力し、終了する
6. $False$ を出力し、終了する
...
ここまでお読みいただいた方の中には、「でも、Miller-Rabin法って、確率的アルゴリズムでしょう。判定が外れる可能性もあるわけで、判定結果の信頼性はどの程度なの？」と思われているかもしれません。

Miller-Rabin 法において、合成数が素数の可能性があると判定されてしまう $a$ の数は、多くても $\frac{n-1}{4}$ であることが知られています。この導出過程は後日追記します。

したがって、ランダムに $k(0 < k < n)$ 個の $a$ を決めてテストを行う場合、全ての $a$ で合成数が素数の可能性があると判定されてしまう確率は

$$
(\frac{\frac{n - 1}{4}}{n - 1})^k = \frac{1}{4^k}
$$

です。これより、$a$ のとる数が少ない場合であっても、十分に信頼できることが分かります。
