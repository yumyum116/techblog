...
## 3. さらに高速に素数判定を行うには
実は、第１章で紹介した `is_prime_sqrt(n)` 関数は、さらに高速化することができます。
...
#### おまけ
本記事でご紹介した３つの異なる素数判定プログラムについて、$n = 99999937$ とした時の実行時間を比較してみました。
...
最後にご紹介した、偶数の判定を skip する関数 `is_prime_even(n)` は、期待していたほど高速化されなかったですね。
もっと大きな値で比較できると差が出たかもしれませんが、`is_prime_n(n)` が TLE になるので、本記事の中では検証しません。
気になる方は、お手元の環境で試してみてください。
